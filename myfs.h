...
private:
    ...
    /**
     * This struct represents a directory entry in myfs filesystem.
     */
    struct myfs_directory_entry {
    ...
    /**
     * This struct represents an inode in myfs filesystem.
     */
    struct myfs_inode {
        uint32_t size;
        bool is_directory;
        // Add additional inode metadata if necessary
    };
    myfs_directory_entry find_entry(std::string path_str);
    int get_next_inode();
